...
public class SignLogController {
    ...
    public List getSignLog(@RequestBody JSONObject param, HttpServletRequest request, HttpServletResponse respons){

        logger.info("-= enter getSignLog =-");

        List<SignLog> signLogs=signLogRepository.findAll();
        List allSignLog=signLogService.getSignLog(request,signLogs);
